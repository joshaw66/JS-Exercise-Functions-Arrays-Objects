...
function getLastCarInfo(inventory) {
  ...
  return `This is a ${thelast.car_make} ${thelast.car_model}`
}
...
function getCarInfoById(inventory, idNum) {
  const findCar = inventory[idNum -1]
    return `The car is a ${findCar.car_make} ${findCar.car_model}`
}
...
*/
function sortCarInventory(cars_inventory) {
  return cars_inventory.sort((car1, car2) => (car1.car_model > car2.car_model) ? 1 : -1)
}
...
function getGermanCars(cars_inventory) {
  ...
  for (let idx = 0; idx < cars_inventory.length; idx++) {
    let car = cars_inventory[idx];
    if (germanMakes.includes(car.car_make)) {
      germanCars.push(car);
    }
  }
  return germanCars;
